...
export function CollectionPointsProvider({ children }: { children: ReactNode }) {
    const [points, setPoints] = useState<CollectionPoint[]>(() => {
        ...
        if (savedPoints) {
            try {
                ...
                const userPoints = parsedPoints.filter((p: CollectionPoint) => !mockPointsIds.has(p.id));
    ...
    const filterPoints = useCallback((query: string) => {
...
export function useCollectionPoints(query: string = '') {
    ...
    if (context === undefined) {
        throw new Error('useCollectionPoints must be used within a CollectionPointsProvider');
    }
